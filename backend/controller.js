...
const seed = (req, res) => {
  sequelize
    ...
    create table recipes (
        ...
        insert into recipes(user_id, recipe_name, recipe_image, time_to_make, servings, ingredient_quantity, instruction)
        VALUES
...
function getRecipeCard(req, res) {
  sequelize
    .query(
      `SELECT recipe_image, recipe_name, ingredient_quantity, instruction
    FROM recipes
    WHERE user_id = 1;`
    )
    .then((dbRes) => res.status(200).send(dbRes[0]))
